...
export default function RegistrationPage(props) {
  const { mainSignUp, validateReferer, checkUserExists, editreferral } = api;
  const [loading, setLoading] = useState(false);
  const useduserReferral = useSelector(
    (state) => state.usedreferralid.usedreferral
  );
  const { t } = i18n;
            ...
            .then((referralInfo) => {
              ...
              if (referralInfo.uid) {
                mainSignUp({
                  ...
                  dispatch(
                    editreferral(
                      { email: regData.email, phone: regData.mobile },
                      "Add"
                    )
                  );
                  setLoading(false);
